...
*
         IF( F.EQ.CZERO ) THEN
            CS = ZERO
            R = DLAPY2( REAL( G ), IMAG( G ) )
*           Do complex/real division explicitly with two real divisions
            D = DLAPY2( REAL( GS ), IMAG( GS ) )
            SN = CMPLX( REAL( GS ) / D, -IMAG( GS ) / D )
            RETURN
         END IF
...
*        Do complex/real division explicitly with 2 real divisions
         ...
         ELSE
            ...
            FF = CMPLX( DR / D, DI / D )
         END IF
         SN = FF*CMPLX( REAL( GS ) / G2S, -IMAG( GS ) / G2S )
         R = CS*F + SN*G
      ELSE
*
...
*        Do the F2S(real)*FS(complex) multiply with two real multiplies
         R = CMPLX( F2S*REAL( FS ), F2S*IMAG( FS ) )
         CS = ONE / F2S
         D = F2 + G2
*        Do complex/real division explicitly with two real divisions
         SN = CMPLX( REAL( R ) / D, IMAG( R ) / D )
         SN = SN*CONJG( GS )
         IF( COUNT.NE.0 ) THEN
            IF( COUNT.GT.0 ) THEN
               DO 30 I = 1, COUNT
