...
c
         do 25 i = 1, nev
            rtemp = max( eps23, dlapy2( real(ritz(np+i)),
     &                                  imag(ritz(np+i)) ) ) 
            if ( dlapy2(real(bounds(np+i)),imag(bounds(np+i))) 
     &                 .le. tol*rtemp ) then
               nconv = nconv + 1
            end if
   25    continue
...
c
            do 35 j = 1, nev0 
                rtemp = max( eps23, dlapy2( real(ritz(j)),
     &                                       imag(ritz(j)) ) )
...
c 
         if (bmat .eq. 'G') then         
            ...
            rnorm = sqrt(dlapy2(real(cmpnorm),imag(cmpnorm)))
         else if (bmat .eq. 'I') then
