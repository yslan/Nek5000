...
c
         if (bmat .eq. 'G') then  
             ...
             wnorm = sqrt( dlapy2(real(cnorm),imag(cnorm)) )
         else if (bmat .eq. 'I') then
...
c
         if (bmat .eq. 'G') then         
            cnorm = zdotc (n, resid, 1, workd(ipj), 1)
            rnorm = sqrt( dlapy2(real(cnorm),imag(cnorm)) )
         else if (bmat .eq. 'I') then
...
c 
         if (bmat .eq. 'G') then         
             cnorm  = zdotc (n, resid, 1, workd(ipj), 1)
             rnorm1 = sqrt( dlapy2(real(cnorm),imag(cnorm)) )
         else if (bmat .eq. 'I') then
...
c              %--------------------------------------------%
c     
               tst1 = dlapy2(real(h(i,i)),imag(h(i,i)))
     &              + dlapy2(real(h(i+1,i+1)), imag(h(i+1,i+1)))
               if( tst1.eq.real(zero) )
     &              tst1 = zlanhs( '1', k+np, h, ldh, workd(n+1) )
               if( dlapy2(real(h(i+1,i)),imag(h(i+1,i))) .le. 
     &                    max( ulp*tst1, smlnum ) ) 
     &             h(i+1,i) = zero
 110        continue
c     
