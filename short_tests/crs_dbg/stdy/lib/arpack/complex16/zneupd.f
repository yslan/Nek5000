...
c
         ...
         do 10 j = 0, ncv-1
            select(j+1) = .false.
            if (which .eq. 'LM') then
               if ( dlapy2(real(workl(irz+j)),
     &                      imag(workl(irz+j))) .ge. thres ) then
                  rtemp = max( eps23, dlapy2(real(workl(irz+j-1)),
     &                         imag(workl(irz+j-1))) )
                  if ( dlapy2(real(workl(ibd+j)),
     &                 imag(workl(ibd+j))) .le. tol*rtemp )
     &               select(j+1) = .true.
               end if 
            else if (which .eq. 'SM') then
               if ( dlapy2(real(workl(irz+j)),
     &                      imag(workl(irz+j))) .le. thres )  then
                  rtemp = max( eps23, dlapy2(real(workl(irz+j-1)),
     &                         imag(workl(irz+j-1))) )
                  if ( dlapy2(real(workl(ibd+j)),
     &                 imag(workl(ibd+j))) .le. tol*rtemp )
     ...
     &               select(j+1) = .true.
               ...
               if ( imag(workl(irz+j)) .ge. thres ) then
                  rtemp = max( eps23, dlapy2(real(workl(irz+j-1)),
     &                    imag(workl(irz+j-1))) )
                  if ( dlapy2(real(workl(ibd+j)),
     &                 imag(workl(ibd+j))) .le. tol*rtemp )
     &               select(j+1) = .true.
               ...
               if ( imag(workl(irz+j)) .le. thres ) then
                  rtemp = max( eps23, dlapy2(real(workl(irz+j-1)),
     &                    imag(workl(irz+j-1))) )
                  if ( dlapy2(real(workl(ibd+j)),
     &                 imag(workl(ibd+j))) .le. tol*rtemp )
