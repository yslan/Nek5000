c     This file is to test the ui of new (anisotropic) term



C     Step 1: scalar only
c        compute f:    tt_crks
c        compute Jac:  tt_cjsks
c        compute prec: tt_scprec

c-----------------------------------------------------------------------
c     input:  rk
c     output: rk = f(rk) (there is a outer routine doing binv and tmask
      subroutine tt_crks(rk) ! compute the kth residual (scalar)
     ...
     $                 u1v(ltd), u2v(ltd), u3v(ltd)
      ...
      if(nio.eq.0) write(*,*)'tt_crks is active'
      ...
      call bcneusc(helm2,-1) ! Neumann/Robin flux
      ...
      call chsign(rk,n)
      call bcneusc(t2,1)  ! Robin source
      call add2(rk,t2,n)
...
c-----------------------------------------------------------------------
c     input:    sk
c     output: J*sk (including P^-1W^-1)
c     
c     For F(u) =  - v dot grad u + div [f(u) grad(u)]
c         J p  =  - v dot grad p + div [f(u) grad(p)] + div [J_f * p grad u]
c              =  
      subroutine tt_cjsks(jsk,sk) ! compute J(u_k) * s_k (coupled)
      implicit none
      include 'SIZE'
      include 'TOTAL'
...
c     call outpost(vx,vy,vz,pr,sk,'js1')
      ...
      call setprop
      call sethlm(helm1,helm2,0)
      call bcneusc(helm2,-1) ! Robin flux

      ceps = 1.E-6
      cinveps = 1./ceps

      ! convection
      call convect_new(jsk,sk,.false.,c1v,c2v,c3v,.true.)
      call axhelm(t2,sk,helm1,helm2,2,1)
      call add2(jsk,t2,n)

      ! linear diffusion
      call add3s2(t2,t,sk,1.0E0,ceps,n) ! t2 = t + eps * sk
      call comp_visc(t1,t2)
      call sub2(t1,helm1,n)
      call cmult(t1,cinveps,n) ! t1 = J_f * sk
      call axhelm(t2,t,t1,helm2,2,1)
      call add2(jsk,t2,n)

      ! nonlinear diffusive induced convection
      call add3s2(t2,t,sk,1.0E0,ceps,n) ! t2 = t + eps * sk
      call comp_q(t1,t2,.true.)
      call sub2(t1,bq(1,1,1,1,ifield-1),n)
      call cmult(t1,cinveps,n) ! t1 = J_f * sk
      call add2(jsk,t1,n)
...
c     input:  v_nt
c     output  z_nt
      subroutine tt_scprec(z,w,ifrefc) ! z = M w
      implicit none
      include 'SIZE'
      include 'TOTAL'
...
c-----------------------------------------------------------------------
      subroutine tt_set_jac_h
c     Set up Jac coef for prec
c        J = - div f(t) grad t - v dot frad t
...
c     
      ...
      real ctemp(lt),c_in(lt),h2temp(lt)
      real ceps,cinveps
      ...
      ifield = 2  ! FIXME, check this later, Lan
      call setprop
      call sethlm(h1,h2,0)
      call bcneusc(h2,-1)
      call rzero(h3,n)
      ...
      call comp_visc(c_in,t)

      ! d/dx
      call add2s1(cx,t,ceps,n) ! tep = t + tx*eps
      call comp_visc(ctemp,cx)
      call sub2(ctemp,c_in,n)
      call cmult(ctemp,cinveps,n)
      ...
      ! d/dy
      call add2s1(cy,t,ceps,n) ! tep = t + ty*eps
      call comp_visc(ctemp,cy)
      call sub2(ctemp,c_in,n)
      call cmult(ctemp,cinveps,n)
      ...
      ! d/dz
      if (ndim.eq.3) then
      call add2s1(cz,t,ceps,n) ! tep = t + tz*eps
      call comp_visc(ctemp,cz)
      call sub2(ctemp,c_in,n)
      call cmult(ctemp,cinveps,n)
      ...
      ! rhs q
      call cadd2(ctemp,t,ceps,n) ! t2 = t + eps * 1
      call comp_q(h2temp,ctemp,.false.)
      call comp_q(ctemp,t,.false.)
      call sub2(h2temp,ctemp,n)
      call cmult(h2temp,cinveps,n)
      call add2(h2,h2temp,n)
      ...
      ifconv = .true.
      call rzero(h2temp,n)
c     call set_up_adf_crs(acrs,h1,h2,h2temp,ifconv,ctemp,c_in)
      ! Set LU solver
      call crs_lu_fact(h1,h2,h2temp,ifconv,ctemp,c_in)
      return
      end
c-----------------------------------------------------------------------
      subroutine comp_visc(visc,t_in)
      implicit none
      include 'SIZE'
      include 'TOTAL'
...
c      call setprop  ! setprop will chk vdiff >0
      MFIELD=2
      IF (IFFLOW) MFIELD=1

      ifld = ifield
      do ifield=mfield,nfield
      do ie=1,nelt
        call nekuvp(ie)
      enddo
      enddo
...
c-----------------------------------------------------------------------
      subroutine comp_q(rhsq,t_in,ifwt) ! userq
      implicit none
      include 'SIZE'
      include 'TOTAL'
...
c=======================================================================
c     Copy from Kento's steady.f and then rename by adding prefix: tt_
c=======================================================================
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      subroutine tt_sns_common ! all common blocks used
...
c-----------------------------------------------------------------------
      subroutine tt_sns_init ! initialize variables to sensible values
...
c-----------------------------------------------------------------------
      subroutine tt_sns_init_max ! initialize variables to sensible values
...
c-----------------------------------------------------------------------
     ...
     $                rtols, atols
      ...
      do isns=1,itn
         ...
         call tt_sns_gmres(sk,rk,vmult,tmult,it)
...
c-----------------------------------------------------------------------
      subroutine tt_cask(ask,sk) ! compute A s_k
...
c-----------------------------------------------------------------------
      subroutine tt_crkf(rk) ! compute the kth residual (fluid)
...
c-----------------------------------------------------------------------
      ...
      if (ifflow) call tt_cfkf(rk)
      if (.not.ifflow) call tt_scnv
      if (ifheat) call tt_cfks(rk(1,itemp))

      return
      end
c-----------------------------------------------------------------------
      subroutine tt_cjsk(jsk,sk)
...
c-----------------------------------------------------------------------
      subroutine tt_cbuoy(buoy,therm)
...
c-----------------------------------------------------------------------
      subroutine tt_cpwf(rk) ! compute the P_w W^-1 on r_k
...
c------------------------------------------------------------------------
      subroutine tt_cwf(rk) ! compute the W^-1 on r_k
...
c------------------------------------------------------------------------
      subroutine tt_cjskf_pr(jsk,sk) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_cjskf(jsk,sk) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_cpws(rk) ! compute the P_w W^-1 on r_k
...
c-----------------------------------------------------------------------
      subroutine tt_ccpsk(cpsk,sk) ! compute C'(u_k) * s_k
...
c-----------------------------------------------------------------------
      subroutine tt_intp_rstd_all(uf,u,nel)
...
c-----------------------------------------------------------------------
      subroutine tt_cpwb_pr(rk) ! compute the P_w W^-1 on r_k
...
c-----------------------------------------------------------------------
      subroutine tt_sns_gmres(phi,res,wt,wtt,iter)
...
c     if (param(51).gt.0.and.isns.gt.2) ifprec = .true.
      ...
      if (ifprec) call tt_set_jac_h
c      if (ifprec.AND.ifflow)call tt_set_jac_h_f
      ifrefc = .true.
      if (sqrt(glsc3(res,res,wt,n)).lt.1.E-8) then 
         ifrefc = .false. ! reduce
      endif
      ...
      do while (iconv.eq.0.and.iter.lt.itmax)
         ...
         if (iter.eq.0) then
            ...
            if (ifflow) then
               ...
               if (ifheat) call tt_cjsks(w_nt(1,itemp),x_nt(1,itemp))
            else
               call tt_cjsks(w_nt,x_nt)
            endif
...
c              if (iter.eq.10) call exitt0
               call tt_cjskf_pr(w_nt,z_nt(1,1,j))
               if (ifheat) call tt_cjsks(w_nt(1,itemp),z_nt(1,itemp,j))
            else
               ifield = 2
               if (ifprec) then
                  tmpk = dnekclock()
                  call tt_scprec(z_nt(1,1,j),v_nt(1,1,j),ifrefc) ! z = M v
                  ifrefc = .false.
                  ptime = ptime + dnekclock() - tmpk
               endif
               call tt_cjsks(w_nt,z_nt(1,1,j))        ! w = A z
            endif
...
c------------------------------------------------------------------------
      subroutine tt_snscmult2(a,b,const)             !  v  = r / gamma
...
c-----------------------------------------------------------------------
      subroutine tt_opcmult2(a,b,const)
...
c-----------------------------------------------------------------------
      subroutine tt_cbad(jsk,sk) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_cjac(jsk,sk) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_setdof(idof,ndof)
...
c------------------------------------------------------------------------
      subroutine tt_setdofp(idof,ndof)
...
c------------------------------------------------------------------------
      subroutine tt_idsop(u,op,nx,ny,nz)
      include 'SIZE'
      include 'PARALLEL'
      include 'INPUT'
      include 'TSTEP'
...
c-----------------------------------------------------------------------
      subroutine tt_ccpsk_bad(cpsk,sk) ! compute C'(u_k) * s_k
...
c-----------------------------------------------------------------------
      subroutine tt_cddd(jsk,sk) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_ceee(jsk,sk,ifcorrect) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_cadv(jsk,sk,ifcorrect) ! compute J(u_k) * s_k (fluid)
...
c-----------------------------------------------------------------------
      subroutine tt_assemble(a,aloc,idof,n,nd,ndof)
...
c-----------------------------------------------------------------------
      subroutine tt_genmat2(aloc,imat,ifexact)
...
c-----------------------------------------------------------------------
      subroutine tt_compact2(a,b,ndof)
...
c-----------------------------------------------------------------------
      subroutine tt_compactv(a,b,ndof)
...
c-----------------------------------------------------------------------
      subroutine tt_matdump0(a,str,nd)
...
c-----------------------------------------------------------------------
      subroutine tt_vecdump(a,str,nd)
...
c-----------------------------------------------------------------------
      subroutine tt_ivecdump(a,str,nd)
...
c-----------------------------------------------------------------------
      subroutine tt_matdump(a,str,nd)
...
c-----------------------------------------------------------------------
      subroutine tt_matdump2(a,str,nd)
...
c-----------------------------------------------------------------------
      subroutine tt_sns_prec_apply(yp,mmat,pvec,idof,ndof)
...
c-----------------------------------------------------------------------
      subroutine tt_sns_prec(a,b) ! a = M b
...
c-----------------------------------------------------------------------
      subroutine tt_sns_prec_init(mmat,pvec,ndof,idof)
...
c-----------------------------------------------------------------------
      subroutine tt_ccsk(csk,sk,ifexact) ! compute C(u_k) * s_k (+ C(s_k)
...
c-----------------------------------------------------------------------
      subroutine tt_checkvec_(sk,dl2)
